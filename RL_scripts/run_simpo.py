...
import torch
import transformers
from transformers import AutoModelForCausalLM, set_seed, T5ForConditionalGeneration


import os
import json
from datasets import DatasetDict, Dataset
def get_moleculeQA_datasets(data_path, cate_name, sample_num):
    raw_datasets = DatasetDict()
    for split in ["train", "test"]:
        with open(os.path.join(data_path, cate_name, split, f"{cate_name}_{sample_num}_corpus.json"), "r") as f:
            data = json.load(f)
...
class SimPOConfig(DPOConfig):
    ...
    ckpt_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to a checkpoint to load model from."},
    )
...
def apply_MoleculeQA_template(example):
    # print(example.keys())
    # for key in example:
    #     print(key)
    #     print(example[key])
    ...
    chosen, rejected, prompt = example["chosen"], example["rejected"], example["prompt"]
...
def main():
    ...
    # )
    raw_datasets = get_moleculeQA_datasets(training_args.data_path, training_args.cate_name, training_args.sample_num)
    logger.info(
    ...
    if training_args.ckpt_path != None:
        ...
        model.load_state_dict(torch.load(training_args.ckpt_path)["model"].state_dict())
        ref_model.load_state_dict(torch.load(training_args.ckpt_path)["model"].state_dict())
        logger.info(f"Load checkpoint {training_args.ckpt_path}")
    #########################
